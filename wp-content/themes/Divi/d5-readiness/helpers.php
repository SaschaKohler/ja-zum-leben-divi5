...
function get_third_party_module_lookup_map() {
	...
	// Get third-party module slugs to limit processing.
	...
	foreach ( $module_collections as $module ) {
		if ( isset( $module['d4Shortcode'] ) && array_key_exists( $module['d4Shortcode'], $third_party_module_slugs ) ) {
...
function is_third_party_module_convertible( $shortcode_name ) {
	...
	// Bail early if the module name is not found.
	if ( ! $module_name ) {
		return false;
	}
