	...
	protected function _check_post_content( $content ) {
		...
		foreach ( $module_slugs as $slug => $module_name ) {
			if ( strpos( $content, $slug ) !== false ) {
				// Skip convertible third-party modules - they don't make pages incompatible.
				if ( array_key_exists( $slug, $third_party_module_slugs ) && Helpers\is_third_party_module_convertible( $slug ) ) {
					continue;
				}

				// Include all other modules (non-convertible third-party, native, WooCommerce).
