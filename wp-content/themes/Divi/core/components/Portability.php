...
class ET_Core_Portability {
	...
	protected static function _get_preset_ids_from_d5_content( $content, $d5_presets ) {
		$patterns = [
			// Test regex: https://regex101.com/r/EmuPwl/1.
	...
	protected function _convert_and_import_d5_presets_from_onboarding( $new_presets_d4, $preset_prefix, $skip_default ) {
		...
		$converted_presets = GlobalPreset::process_presets( $converted_presets );
	...
	/**
	 * Convert and import D5 presets during import.
	 * 1. Convert new presets data if needed.
	 * 2. Process D5 presets and merge with existing presets.
	 * 3. Save all presets data.
	 *
	 * @since ??
	 *
	 * @param array $new_presets_d4 Array of new presets data.
	 */
	protected function _save_d5_presets( $new_presets_d4 ) {
		$converted_presets    = D5BuilderConversion::maybe_convert_presets_data( $new_presets_d4 );
		$processed_presets_d5 = GlobalPreset::process_presets( $converted_presets );

		GlobalPreset::save_data( $processed_presets_d5 );
	}
	...
	public function import_global_presets(
		...
		if ( isset( $presets['module'] ) || isset( $presets['group'] ) ) {
			$new_module_presets = $presets['module'] ?? [];
			$new_group_presets  = $presets['group'] ?? [];

			// Get existing presets.
			$all_presets = GlobalPreset::get_data();

			// Merge module presets.
			if ( ! empty( $new_module_presets ) ) {
				$all_presets['module'] = array_replace_recursive(
					$all_presets['module'] ?? [],
					$new_module_presets
				);
			}
			...
			// Merge group presets.
			if ( ! empty( $new_group_presets ) ) {
				$all_presets['group'] = array_replace_recursive(
					$all_presets['group'] ?? [],
					$new_group_presets
				);
			}

			// Save the final combined presets.
			GlobalPreset::save_data( $all_presets );
		...
		$this->_save_d5_presets( $new_presets_d4 );
		...
		foreach ( [ 'module', 'group' ] as $component_type ) {
			...
			foreach ( $global_presets[ $component_type ] as $category => &$category_data ) {
				...
				foreach ( $items as $preset_id => $preset ) {
					...
					$preset_data['name']       = ! empty( $existing_preset ) || $is_default_preset
...
function et_core_portability_ajax_import() {
	...
	if ( ! $result = $portability->import() ) {
		...
		if ( $replace && $post_id > 0 && current_user_can( 'edit_post', $post_id ) ) {
				...
				$d5_content = wp_slash( D5BuilderConversion::maybeConvertContent( $content ) );
				// phpcs:disable -- It's old code and phpcs is not working properly.
				wp_update_post(
					array(
					'ID'           => $post_id,
					'post_content' =>"<!-- wp:divi/placeholder -->{$d5_content}<!-- /wp:divi/placeholder -->",
