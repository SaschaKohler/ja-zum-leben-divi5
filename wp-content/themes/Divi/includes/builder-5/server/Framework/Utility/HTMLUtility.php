...
class HTMLUtility {
	...
	public static function is_valid_attribute( $attribute, $tag ) {
		...
		// Set custom sanitizer for `href` attribute to preserve dynamic content variables.
		if ( $details && 'href' === $attribute ) {
			$details['sanitizer'] = function( $value ) {
				// Preserve dynamic content variables, sanitize everything else.
				if ( false !== strpos( $value, '$variable(' ) ) {
					return $value; // Preserve JSON variables for later processing.
				}
				return esc_url( $value ); // Normal URL sanitization.
			};
		}
