...
class MediaUtility {
	...
	public static function get_image_sizes_options(): array {
		...
		// Add full size first.
		$sizes['full'] = esc_html__( 'Full Size (Original)', 'et_builder' );
			...
			if ( ! empty( $size_data ) ) {
				...
				switch ( $size ) {
					case 'thumbnail':
						...
						$sizes[ $size ] = sprintf( '%s (%dx%d)', esc_html__( 'Thumbnail', 'et_builder' ), $width, $height );
						break;
					case 'medium':
						$width          = get_option( 'medium_size_w', 300 );
						$height         = get_option( 'medium_size_h', 300 );
						$sizes[ $size ] = sprintf( '%s (%dx%d)', esc_html__( 'Medium', 'et_builder' ), $width, $height );
						break;
					case 'large':
						$width          = get_option( 'large_size_w', 1024 );
						$height         = get_option( 'large_size_h', 1024 );
						$sizes[ $size ] = sprintf( '%s (%dx%d)', esc_html__( 'Large', 'et_builder' ), $width, $height );
						break;
					default:
