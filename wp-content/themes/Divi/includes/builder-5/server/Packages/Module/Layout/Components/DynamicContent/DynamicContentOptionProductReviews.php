...
class DynamicContentOptionProductReviews extends DynamicContentOptionBase implements DynamicContentOptionInterface {
	...
	public function render_callback( $value, array $data_args = [] ): string {
		...
		if ( $post ) {
			$dynamic_product = \ET\Builder\Packages\WooCommerce\WooCommerceUtils::get_product( $post_id );
			$value           = '';
			...
			if ( $dynamic_product && comments_open( $dynamic_product->get_id() ) ) {
				...
				$reviews_title        = \ET\Builder\Packages\WooCommerce\WooCommerceUtils::get_reviews_title( $dynamic_product );
				$reviews_comment_form = \ET\Builder\Packages\WooCommerce\WooCommerceUtils::get_reviews_comment_form( $dynamic_product, $comments );
