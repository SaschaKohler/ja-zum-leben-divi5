...
class Module {
	...
	public static function render( array $args ): string {
		...
		// Inject loop iteration into module attributes for consistent loop handling across all modules.
		$attrs_with_loop_context = $attrs;
		$loop_context            = LoopContext::get();

		if ( $loop_context ) {
			$current_iteration                           = $loop_context->get_current_iteration();
			$attrs_with_loop_context['__loop_iteration'] = $current_iteration;
		}
		...
		// Add flex module class when layout is flex.
		// Only check for flex layout if the module actually has the layout option group configured.
		...
		// Generate CSS variable declarations from background image attributes.
		$css_declarations = self::_generate_css_declarations_from_background_image_attrs( $attrs );
		if ( ! empty( $css_declarations ) ) {
			// Merge CSS declarations into styles array.
			$styles = array_merge( $styles, $css_declarations );
		}
		...
		$loop_iteration = $attrs_with_loop_context['__loop_iteration'] ?? null;
	...
	/**
	 * Generates CSS variable declarations from module background image attributes.
	 *
	 * This function searches for image URLs in the background decoration attributes
	 * across all available breakpoints and generates ready-to-use CSS declarations.
	 *
	 * @param array $attrs The module attributes.
	 * @return array Array of CSS variable declarations (e.g., ["--loop_post_featured_image:url(...);"])
	 */
	private static function _generate_css_declarations_from_background_image_attrs( array $attrs ): array {
		$css_declarations = [];
		$seen_variables   = [];

		// Special case for counter module.
		$background_path = isset( $attrs['barCounter'] ) ? 'barCounter' : 'module';

		// Check if background decoration exists.
		$background_attr = $attrs[ $background_path ]['decoration']['background'] ?? null;
		if ( ! $background_attr ) {
			return $css_declarations;
		}

		// Iterate through all available breakpoints dynamically.
		foreach ( $background_attr as $breakpoint_data ) {
			$attr_value = $breakpoint_data['value'] ?? null;
			if ( ! $attr_value ) {
				continue;
			}

			// Check if image URL exists.
			$image_variable = $attr_value['image']['url'] ?? null;
			if ( ! $image_variable ) {
				continue;
			}

			// Skip if we've already seen this URL.
			if ( in_array( $image_variable, $seen_variables, true ) ) {
				continue;
			}

			$seen_variables[] = $image_variable;

			// Extract CSS variable name if URL contains $variable syntax.
			$css_variable_name = self::_extract_css_variable_name( $image_variable );
			...
			$css_declarations[] = "--{$css_variable_name}:url({$image_variable});";
		}
	...
	/**
	 * Extracts the CSS variable name from a $variable() syntax string.
	 *
	 * This function reuses existing DynamicData utilities to avoid code duplication.
	 *
	 * @param string $url The URL that may contain $variable syntax.
	 * @return string|null The extracted variable name or null if not found.
	 */
	private static function _extract_css_variable_name( string $url ): ?string {
		// Check if the URL contains $variable syntax.
		if ( false === strpos( $url, '$variable(' ) ) {
			return null;
		}

		// Use existing DynamicData utility to extract variable JSON content.
		$variable_values = DynamicData::get_variable_values( $url );

		if ( empty( $variable_values ) ) {
			return null;
		}

		// Parse the first variable JSON content since we only support one variable at a time.
		$decoded = json_decode( $variable_values[0], true );
