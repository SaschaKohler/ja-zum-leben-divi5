		...
		switch ( $meta_type ) {
			case 'post':
				...
				$sql = "SELECT DISTINCT pm.meta_key FROM {$wpdb->postmeta} pm
						...
						WHERE pm.meta_key NOT IN (
							SELECT DISTINCT pm2.meta_key FROM {$wpdb->postmeta} pm2
							WHERE pm2.meta_value REGEXP '^(a:|O:|C:)'
						)
			...
			case 'user':
				$sql = "SELECT DISTINCT um.meta_key FROM {$wpdb->usermeta} um
						INNER JOIN {$wpdb->users} u ON ( u.ID = um.user_id )
						WHERE um.meta_key NOT LIKE '%capabilities'
						AND um.meta_key NOT LIKE '%user_level'
						AND um.meta_key NOT LIKE '%session_tokens'
						AND um.meta_key NOT IN (
							SELECT DISTINCT um2.meta_key FROM {$wpdb->usermeta} um2
							WHERE um2.meta_value REGEXP '^(a:|O:|C:)'
						)
						GROUP BY um.meta_key
						ORDER BY COUNT(um.meta_key) DESC";
				break;
			...
			case 'term':
				...
				$sql = "SELECT DISTINCT tm.meta_key FROM {$wpdb->termmeta} tm
						INNER JOIN {$wpdb->terms} t ON ( t.term_id = tm.term_id )
						INNER JOIN {$wpdb->term_taxonomy} tt ON ( tt.term_id = t.term_id AND tt.taxonomy IN ({$taxonomies}) )
						WHERE tm.meta_key NOT IN (
							SELECT DISTINCT tm2.meta_key FROM {$wpdb->termmeta} tm2
							WHERE tm2.meta_value REGEXP '^(a:|O:|C:)'
						)
						GROUP BY tm.meta_key
						ORDER BY COUNT(tm.meta_key) DESC";
				break;
