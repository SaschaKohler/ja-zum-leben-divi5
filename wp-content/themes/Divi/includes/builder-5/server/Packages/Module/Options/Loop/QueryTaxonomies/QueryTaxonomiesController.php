...
class QueryTaxonomiesController extends RESTController {
	...
	public static function index_args(): array {
		return [
			'post_type' => [
				'description'       => __( 'Comma-separated list of post types to get taxonomies for. If empty, returns taxonomies for all post types.', 'et_builder' ),
				'type'              => 'string',
				'required'          => false,
				'default'           => '',
					...
					if ( is_string( $param ) ) {
						...
						foreach ( $post_types as $post_type ) {
							if ( ! post_type_exists( $post_type ) ) {
								return new \WP_Error(
									'invalid_post_type',
									sprintf( __( 'Post type "%s" does not exist.', 'et_builder' ), $post_type )
								);
							}
						}
						return true;
