			...
			foreach ( Breakpoint::get_enabled_breakpoint_names() as $breakpoint ) {
				...
				if ( is_array( $state_values ) ) {
					...
					foreach ( ModuleUtils::states() as $state ) {
						...
						if ( ( 'on' === $current_enabled || ( is_null( $current_enabled ) && 'on' === $larger_breakpoint_enabled ) ) && isset( $enable[ $state ] ) && $enable[ $state ] && ( ( '' === $current_url ) || ( $current_url ?? $larger_breakpoint_url ) ) ) {
