...
class WooCommerceHooks implements DependencyInterface {
	...
	public function load(): void {
		...
		// Remove default notices on product single pages.
		add_action(
	 ...
	 *
	 * NOTE: Once we have the cart and checkout modules, we need to refactor this function.
	 *
	 ...
	 * @return bool
	 */
	public static function should_replace_content( $shortcode ): bool {
	 ...
	 *
	 * NOTE: Once we have the cart modules, we need to refactor this function.
	 *
	 * @since ??
	 *
	 * @return string
	...
	public static function get_prefilled_cart_page_content(): string {
		if ( ! et_is_shortcode_framework_loaded() && et_is_woocommerce_plugin_active() ) {
	 ...
	 *
	 * NOTE: Once we have the checkout modules, we need to refactor this function.
	 *
	 * @since ??
	 * @return string
	 */
	...
	public static function set_prefilled_page_content( string $maybe_builder_content, int $post_id ): string {
		...
		if ( $is_cart && $should_replace_content ) {
			...
			$has_et_builder_shortcode          = has_shortcode( $maybe_builder_content, 'et_pb_section' );
	...
	public static function template_loop_product_thumbnail(): void {
		...
		if ( ! empty( $overlay_attr ) ) {
			$overlay_class = \ET\Builder\Packages\Module\Options\BoxShadow\BoxShadowClassnames::has_overlay( $overlay_attr );

			if ( $overlay_class ) {
				$overlay_component = \ET\Builder\Packages\Module\Options\BoxShadow\BoxShadowOverlay::component( [ 'attr' => $overlay_attr ] );
