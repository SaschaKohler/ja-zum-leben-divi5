...
class Conversion {
	...
	static function convertGlobalColor( $encodedValue, $name, $moduleGlobalColors ) {
		...
		// convert it to a CSS variable to make it editable in ColorPicker components
	...
	static function encodeAttrs(array $attrs, string $moduleName, ?string $content = null, $originalShortcode = null, $shortcodeName = null):string {
	...
	static function convertAttrs(array $attrs, string $moduleName, ?string $content = null):array {
		...
		if (isset($convertedAttrs['unknownAttributes']) && is_array($convertedAttrs['unknownAttributes'])) {
			...
			// attrName to ignore => array of attrs and their values that must be present to ignore the unknown attribute.
			$ignoredUnknownAttrs = [
				// Popups For Divi extension adds this attr with a default value to all sections regardless
				// of whether or not popup is enabled for the section.
				'da_disable_devices' => ['da_is_popup' => 'on'],
			];
			...
			foreach ($currentUnknownAttrs as $unknownAttr) {
				...
				if (LegacyAttributeNames::is_legacy_attribute($unknownAttr)) {
					continue; // Skip legacy attribute names
				}

				// For attributes in the ignored list, we want to KEEP them only if all conditions are met
				// Otherwise, we'll treat them as known attributes (i.e., remove them)
				if (isset($ignoredUnknownAttrs[$unknownAttr]) && is_array($ignoredUnknownAttrs[$unknownAttr])) {
					$conditions = $ignoredUnknownAttrs[$unknownAttr];
					$allConditionsMet = true;

					foreach ($conditions as $conditionAttr => $expectedValue) {
						if (! isset($convertedAttrs[$conditionAttr]) || $convertedAttrs[$conditionAttr] !== $expectedValue) {
					...
					if (! $allConditionsMet) {
						continue; // Skip this unknown attribute if conditions are NOT met.
					}
				}
				...
				// If the unknown attribute is not known in other modules, keep it.
