...
class WooCommerceProductTitleModule implements DependencyInterface {
	...
	public static function render_callback( array $attrs, string $content, WP_Block $block, ModuleElements $elements ): string {
		...
		$heading_level   = $attrs['content']['advanced']['headingLevel']['desktop']['value'] ?? 'h1';
	...
	public static function get_title( array $args = [] ): string {
		...
		// When 'product' attribute is not set or it is set to 'current', the module is intended to display
		// the title of the current product from the loop or the main post. This is the "dynamic" context.
		// The `DynamicContentPosts::get_current_page_title()` handles this case correctly, including
		// showing the appropriate placeholder in the Theme Builder preview.
