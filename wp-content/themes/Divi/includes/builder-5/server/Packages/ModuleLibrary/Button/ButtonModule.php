...
class ButtonModule implements DependencyInterface {
	...
	public static function render_callback( array $attrs, string $content, WP_Block $block, ModuleElements $elements, array $default_printed_style_attrs ): string {
		...
		// ButtonModule bypasses elements->render() because it needs separate access to linkUrl,
		// and text values for different purposes (href vs display text), plus custom fallback logic.
		// Since this bypasses automatic dynamic content processing, we manually handle loop variables.
		$link_value = LoopUtils::process_dynamic_content( $link_value, $attrs );
		$text_value = LoopUtils::process_dynamic_content( $text_value, $attrs );
		...
		return Module::render(
			[
				...
				'elements'                  => $elements,
				'htmlAttrs'                 => [
					'href'             => et_core_esc_previously( $link_value ),
