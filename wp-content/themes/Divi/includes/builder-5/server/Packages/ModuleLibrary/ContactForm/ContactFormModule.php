										...
										'hover' => [
											'background-color' => implode(
												...
												[
													...
													"{$order_class} .input[type=\"checkbox\"] + label:hover i",
													"{$order_class} .input[type=\"radio\"] + label:hover i",
												]
											),
										],
									],
		...
		if ( $form_handler->is_submitted() ) {
			if ( $form_handler->get_error()->has_errors() ) {
				...
				if ( $form_handler->is_mail_sent() ) {
					...
					if ( '' === $success_message ) {
						$success_message = __( 'Thanks for contacting us', 'et_builder' );
					}
					...
					$message_children = HTMLUtility::render(
						[
							...
							'children'   => __( 'There was an error trying to send your message. Please try again later.', 'et_builder' ),
						]
					);
				}
