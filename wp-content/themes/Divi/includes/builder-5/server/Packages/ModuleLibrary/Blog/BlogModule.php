...
class BlogModule implements DependencyInterface {
	...
	public static function blog_grid_item_style_declaration( array $params ): string {
		...
		// Extract values from nested breakpoint structure.
		$layout_value = $attr['desktop']['value'] ?? [];
		$display      = $layout_value['display'] ?? 'flex';
		$column_gap   = $layout_value['columnGap'] ?? '';

		// Add grid item styles when horizontal gap is set for blog grid.
		if ( ( is_string( $column_gap ) && '' !== $column_gap ) && 'block' !== $display ) {
			$declarations->add( '--row-gutter-horizontal', $column_gap );
		}
	...
	public static function render_callback( $attrs, $content, $block, $elements ) {
		...
		$layout_display  = $attrs['blogGrid']['decoration']['layout']['desktop']['value']['display'] ?? 'flex';
		...
		if ( $is_block_layout ) {
			...
			// Add et_flex_grid class to et_pb_posts when layout style is flex.
			$content_wrapper_attributes = [
				'class' => 'et_pb_posts et_flex_grid',
			];
		}

		// Build container class - add et_pb_posts and et_flex_grid when layout is flex.
		$container_class = $is_block_layout
			? 'et_pb_ajax_pagination_container et_pb_posts'
			: 'et_pb_ajax_pagination_container et_pb_posts et_flex_grid';
	...
	public static function process_post_data( \WP_Post $post, array $attrs, int $order_index, int $item_index ): string {
		...
		// Generate responsive flexType column classes when layout is flex.
		$flex_classes = [];

		// Check if layout display is set to flex.
		$blog_grid_layout = $attrs['blogGrid']['decoration']['layout']['desktop']['value']['display'] ?? 'flex';
		$is_flex_layout   = 'flex' === $blog_grid_layout;

		if ( $is_flex_layout ) {
			// Add generic flex column class for both Grid and Fullwidth modes.
			$flex_classes[] = 'et_flex_column';

			if ( 'off' === $fullwidth && isset( $attrs['blogGrid']['advanced']['flexType'] ) ) {
				// Grid mode: Apply responsive flexType column classes.
				$breakpoints = [
					'desktop'    => '',
					'tablet'     => '_tablet',
					'phone'      => '_phone',
					'phoneWide'  => '_phone_wide',
					'tabletWide' => '_tablet_wide',
					'widescreen' => '_widescreen',
					'ultraWide'  => '_ultrawide',
				];

				foreach ( $breakpoints as $breakpoint => $suffix ) {
					$flex_type = $attrs['blogGrid']['advanced']['flexType'][ $breakpoint ]['value'] ?? null;
					if ( $flex_type && 'none' !== $flex_type ) {
						$flex_classes[] = "et_flex_column_{$flex_type}{$suffix}";
					}
				}
			} elseif ( 'on' === $fullwidth ) {
				// Fullwidth mode: Apply full width column class.
				$flex_classes[] = 'et_flex_column_24_24';
			}
		}
