...
class WooCommerceProductsModule implements DependencyInterface {
	...
	public static function overflow_style_declaration( array $params ): string {
		...
		foreach ( $radius as $corner => $value ) {
			...
			// Resolve global variables before parsing numeric values.
			$resolved_value = Utils::resolve_dynamic_variable( $value ?? '' );
			if ( StringUtility::starts_with( $resolved_value, 'var(--' ) ) {
				$all_corners_zero = false;
				break;
			}

			$corner_value = SanitizerUtility::numeric_parse_value( $resolved_value );
