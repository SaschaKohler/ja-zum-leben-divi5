...
class FilterablePortfolioModule implements DependencyInterface {
	...
	public static function filterable_portfolio_grid_item_style_declaration( array $params ): string {
		...
		// Add grid item styles when horizontal gap is set for filterable portfolio grid.
	...
	public static function module_styles( array $args ): void {
		...
		// Check layout to determine when portfolioGrid styles should be applied.
		$layout              = $attrs['portfolio']['advanced']['layout']['desktop']['value'] ?? 'on';
	...
	public static function render_callback( array $attrs, string $content, WP_Block $block, ModuleElements $elements, array $default_printed_style_attrs ): string {
		...
		foreach ( $filterable_portfolio_posts as $portfolio ) {
			...
			if ( ! $is_layout_fullwidth && $is_flex_layout ) {
				...
				$portfolio_item_classes[] = 'et_flex_column';
				...
				foreach ( $breakpoints_mapping as $breakpoint => $suffix ) {
					...
					if ( $flex_type && 'none' !== $flex_type ) {
						$portfolio_item_classes[] = "et_flex_column_{$flex_type}{$suffix}";
					}
				}
			}
		...
		$portfolio_items_classes = [ 'et_pb_portfolio_items' ];

		if ( $is_flex_layout ) {
			$portfolio_items_classes[] = 'et_flex_grid';
		}
