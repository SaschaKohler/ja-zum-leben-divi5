...
class PortfolioModule implements DependencyInterface {
	...
	public static function portfolio_grid_item_style_declaration( array $params ): string {
		...
		// Add grid item styles when horizontal gap is set for portfolio grid.
	...
	public static function render_callback( array $attrs, string $content, WP_Block $block, ModuleElements $elements, array $default_printed_style_attrs ): string {
		...
		if ( $query->have_posts() ) {
			while ( $query->have_posts() ) {
				...
				// Add flex column class for grid items when layout is flex.
				$portfolio_grid_layout = $attrs['portfolioGrid']['decoration']['layout']['desktop']['value'] ?? null;
				$is_flex_layout        = 'flex' === ( ( $portfolio_grid_layout ?? [] )['display'] ?? 'flex' );
				if ( ! $is_layout_fullwidth && $is_flex_layout ) {
					// Add generic flex column class.
					$post_classes[] = 'et_flex_column';
					...
					foreach ( $breakpoints_mapping as $breakpoint => $suffix ) {
						...
						$flex_type = $attrs['portfolioGrid']['advanced']['flexType'][ $breakpoint ]['value'] ?? null;

						if ( $flex_type && 'none' !== $flex_type ) {
							$post_classes[] = "et_flex_column_{$flex_type}{$suffix}";
						}
					}
				}
		 ...
		 */
		// Portfolio Grid Layout (Flex Settings) for wrapper classes.
		$portfolio_grid_layout  = $attrs['portfolioGrid']['decoration']['layout']['desktop']['value']['display'] ?? 'flex';
		$is_flex_layout         = 'flex' === $portfolio_grid_layout;
		$portfolio_grid_classes = [ 'et_pb_portfolio_grid_items', 'et_pb_grid_items' ];

		if ( $is_flex_layout ) {
			$portfolio_grid_classes[] = 'et_flex_grid';
		}

		$portfolio_grid_wrapper = $is_layout_fullwidth ? '' : sprintf( '<div class="%s">', esc_attr( implode( ' ', $portfolio_grid_classes ) ) );
		...
		$children = $rendered_post ? strtr(
			...
			[
				'{{wrapper_open}}'  => $portfolio_grid_wrapper,
