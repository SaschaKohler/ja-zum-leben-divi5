...
class AudioModule implements DependencyInterface {
	...
	public static function render_callback( array $attrs, string $content, WP_Block $block, ModuleElements $elements, array $default_printed_style_attrs ): string {
		...
		$is_no_results_content = ! empty( $content ) && false !== strpos( $content, 'No Results Found.' );

		if ( $is_no_results_content ) {
			$audio_content = et_core_esc_previously( $content );
		} else {
			remove_all_filters( 'wp_audio_shortcode_library' );
			remove_all_filters( 'wp_audio_shortcode' );
			remove_all_filters( 'wp_audio_shortcode_class' );

			$audio_content = et_core_esc_previously( $title ) . et_core_esc_previously( $meta ) . et_core_esc_previously( do_shortcode( sprintf( '[audio src="%s" /]', esc_url( $audio ) ) ) );
		}
