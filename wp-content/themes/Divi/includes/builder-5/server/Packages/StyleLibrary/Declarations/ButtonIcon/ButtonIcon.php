...
class ButtonIcon {
	...
	/**
	 * Get icon margin based on placement and hover state.
	 *
	 * @since ??
	 *
	 * @param string $placement Icon placement ('left' or 'right').
	 * @param string $on_hover  Hover state ('on' or 'off').
	 *
	 * @return string Icon margin value.
	 */
	public static function get_icon_margin( $placement, $on_hover ) {
		// Left placement always uses negative margin.
		if ( 'left' === $placement ) {
			return '-1.3em';
		}

		// Right placement with hover on uses negative margin.
		if ( 'right' === $placement && 'on' === $on_hover ) {
			return '-1.3em';
		}

		// Right placement with hover off uses positive margin.
		return '.3em';
	}
	...
	public static function style_declaration( array $args ) {
		...
		if ( 'on' === $enable ) {
			...
			$icon_margin = self::get_icon_margin( $placement, $on_hover );
