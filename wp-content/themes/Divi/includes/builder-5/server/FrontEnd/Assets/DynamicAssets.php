...
class DynamicAssets implements DependencyInterface {
	...
	public function pre_initial_setup() {
		...
		if ( ! DynamicAssetsUtils::should_initiate_dynamic_assets() ) {
			$block_names = DetectFeature::get_block_names( $this->_all_content );

			// Check whether the content have shortcodes.
			if ( DetectFeature::get_shortcode_names( $this->_all_content ) ) {
	...
	public function get_global_assets_list(): array {
		...
		if ( ! $this->_use_global_colors ) {
			...
			$_early_global_color_ids = DetectFeature::get_global_color_ids( $this->_all_content . wp_json_encode( $page_setting_attributes ) );

			// Merge global color ids from the content and page settings, ensuring that global colors from `Customizer` is always included.
		...
		$block_used = DetectFeature::has_block_layout_enabled( $this->_all_content, $this->_options );
		...
		$responsive_breakpoints = DetectFeature::get_flex_grid_responsive_breakpoints( $this->_all_content, $this->_options );
		...
		$flex_used = DetectFeature::has_flex_layout_enabled( $this->_all_content, $this->_options );
		...
		$show_in_lightbox = DetectFeature::has_lightbox( $this->_all_content, $this->_options ) || DetectFeature::has_3p_lightbox( $this->_all_content, $this->_options );
		...
		$has_block_mode_blog = DetectFeature::has_block_mode_blog_enabled( $this->_all_content, $this->_options );
	...
	public function get_block_assets_list( bool $used_modules = true ): array {
		...
		if ( DetectFeature::has_block_layout_enabled( $this->_all_content, $this->_options ) && isset( $assets_list['divi/row'] ) ) {
	...
	public function get_late_attributes() {
		...
		// Performance optimization: Early exit only if no late detection is needed.
		if ( empty( $this->_missed_blocks ) && empty( $this->_missed_shortcodes ) && ! $this->_early_attributes ) {
			// No missed modules and no early attributes - completely skip late processing.
			return;
		}
	...
	public function enqueue_dynamic_scripts( string $request_type = 'early' ) {
		...
		if ( ! $this->_enqueue_magnific_popup ) {
			...
			if ( ! $this->_enqueue_magnific_popup && DetectFeature::has_3p_lightbox( $this->_all_content, $this->_options ) ) {
				$this->_enqueue_magnific_popup = true;
			}
		...
		if ( ! $this->_enqueue_salvattore ) {
			// Only enqueue salvattore when we have a block mode blog.
			$this->_enqueue_salvattore = DetectFeature::has_block_mode_blog_enabled( $this->_all_content, $this->_options ) || $this->_late_use_block_mode_blog;
		...
		if ( 'late' === $request_type ) {
			...
			if ( DetectFeature::has_interactions_enabled( $this->_all_content, $this->_options ) ) {
