...
class DetectFeature {
	...
	public static function has_interactions_enabled( string $content, array $options = [] ): bool {
		...
		if ( ! empty( $options['has_block'] ) ) {
			...
			// test regex: https://regex101.com/r/8jBF7q/1.
			$block_pattern = '/"decoration":.*?"interactions":\s*\[\s*{/';
			if ( preg_match( $block_pattern, $content ) ) {
				return true;
			}
		}
	...
	public static function has_block_layout_enabled( string $content, array $options = [] ): bool {
		 ...
		 * test regex: https://regex101.com/r/KRltF1/1
		 */
		static $pattern = '"layout":{.*?"display":"block".*?}';
	 ...
	 *
	 * Flex layout is enabled when flexColumnStructure or flexType attributes are present
	 * in the advanced settings. This feature is used in Divi 5 blocks for flexible
	 * column structures and responsive layouts.
	 *
	 * @since ??
	 *
	...
	public static function has_flex_layout_enabled( string $content, array $options = [] ): bool {
		...
		/*
		 * The pattern to search for flex layout in Gutenberg content.
		 * Flex layout is indicated by flexColumnStructure or flexType attributes,
		 * but only when the block doesn't have "display":"block" in its layout settings.
		 * test regex: https://regex101.com/r/NESfJ5/1.
		 */
		static $pattern = '<!-- wp:[^>]+(?=.*?"flex(ColumnStructure|Type)")(?!.*?"display"\s*:\s*"block")[^>]*-->';

		// Use block-by-block analysis instead of lookaheads.
		preg_match_all( '/<!-- wp:[^>]*-->/', $content, $block_matches );

		foreach ( $block_matches[0] as $block ) {
			// Check if this block has flex attributes.
			if ( preg_match( '/"flex(ColumnStructure|Type)"/', $block ) && ! preg_match( '/"display"\s*:\s*"block"/', $block ) ) {
				return true; // Found flex layout without block display.
			}
		}
	...
	/**
	 * Retrieves the responsive breakpoints that have custom flexColumnStructure defined.
	 *
	 * This method detects which breakpoints (excluding desktop) have custom
	 * flexColumnStructure values defined in the content.
	 *
	 * @since ??
	 *
	 ...
	 * }
	 *
	 * @return array Array of breakpoint names that have custom flexColumnStructure.
	 */
	public static function get_flex_grid_responsive_breakpoints( string $content, array $options = [] ): array {
		// Bail early if content is empty, no blocks are present, or no flex attributes exist.
		if (
			empty( $content )
			|| empty( $options['has_block'] )
			|| ( false === strpos( $content, '"flexColumnStructure"' ) && false === strpos( $content, '"flexType"' ) )
		) {
			return [];
		}
		...
		foreach ( $default_style_breakpoint_order as $breakpoint ) {
			...
			// This regex pattern matches JSON structures where `flexColumnStructure` or `flexType`
			// defines custom values for specific breakpoints (e.g., tablet, phone, etc.).
			// It looks for patterns like: "flexType":{..."tablet":{"value":"..."
			//
			...
			//
			// Each breakpoint requires its own regex pattern because the original regex only captures the first match.
			// This becomes a problem when a single Gutenberg block contains multiple breakpoints,
			// as shown in this example where only the phone breakpoint is captured:
			// https://regex101.com/r/gPqNJg/2.
			$regex = '/"(?:flexColumnStructure|flexType)":\{.*?"(' . $breakpoint . ')":\{"value":"/';

			// If this breakpoint has custom flex settings, add it to our collection.
	...
	public static function has_block_mode_blog_enabled( string $content, array $options = [] ): bool {
		...
		/*
		 * Single regex pattern to detect blog module blocks with block mode enabled.
		 * Matches blocks that contain both:
		 * - fullwidth enable set to "off"
		 * - blogGrid layout display set to "block"
		 * test regex: https://regex101.com/r/TC6RfT/1
		 */
		static $pattern = '/<!-- wp:divi\/blog\s+{.*?"fullwidth":\s*{.*?"enable":\s*{.*?"value":\s*"off".*?}.*?}.*?"blogGrid":\s*{.*?"layout":\s*{.*?"display":\s*"block".*?}.*?}.*?} \/-->/s';

		// Perform a single regex search for block mode blog.
		preg_match( $pattern, $content, $matches );

		// Return true if any block mode blog was detected, false otherwise.
		return ! empty( $matches );
	}
}
